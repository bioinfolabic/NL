...
/* Alloc Arrays */
/* Alloc molecules, energies and neighbourhood list */
void allocArrays() {
...
/* Alloc GPU memory to molecules, energies and the neighbourhood list*/
void allocDevice() {
...
__global__ void cudaLJEnergy(Particle *particles, int *d_neighbour_list, char *d_sequence, double LV, double *d_uLJVector, int n_mol) {
	...
	if (i<n_mol)
	{
		for(int j=(i+2); (j < n_mol) ;j++) {
			...
			/* Neighbourhood list application */
			/* Jump the iterations with molecules that are not neighbours */
				if(d_neighbour_list[neihbor_aux+j]==0)
				{
					continue;
				}						
...
void step() {
	...
	//LJ Energy
	cudaLJEnergy <<<n_blocks, blockSize>>> (d_particles, d_neighbour_list, d_sequence, LV,d_uLJVector, n_mol);
	cudaDeviceSynchronize();
...
/* Update the neighbourhood list in GPU */
/* Function that handles the computation of the neighbourhood list */
__global__ void cudaUpdateNeighbourList (Particle *particles,int *neighbour_list,double r_cut, double skin ) {
	...
	while(i<d_nmol)
	{
			dr1.x = particles[id].v_r.x - particles[i].v_r.x;
			dr1.y = particles[id].v_r.y - particles[i].v_r.y;
			dr1.z = particles[id].v_r.z - particles[i].v_r.z;
			dist = dr1.x * dr1.x + dr1.y * dr1.y + dr1.z * dr1.z;
			dist = sqrt(dist);
			...
			/* = 0 is not neighbour */
			if(dist<(r_cut+skin)){
				neighbour_list[p_1id]=1;
				neighbour_list[p_2id]=1;
			}
			else {
				neighbour_list[p_1id]=0;
				neighbour_list[p_2id]=0;
			}
...
/* Updeate Neighbourhood List */
/* Function that handle the call of the Update of the neighbourhood list in GPU */
/* see function (cudaUpdateNeighbourList()) */
void *updateNeighbourList() {

	cudaSetDevice(device);
	cudaMemcpy(d_particles, particles,n_mol * sizeof(Particle), cudaMemcpyHostToDevice);

	cudaUpdateNeighbourList<<<n_blocks,BLOCKSIZE>>>(d_particles,d_neighbour_list,r_cut,skin);
	cudaDeviceSynchronize();

	cudaMemcpy(neighbour_list, d_neighbour_list, n_mol *n_mol * sizeof(int), cudaMemcpyDeviceToHost);
